...
class MyAcademic extends StatefulWidget {
  const MyAcademic({Key? key}) : super(key: key);

  @override
  State<MyAcademic> createState() => _MyAcademicState();
}
bool updatea = true;
int mycindexa = 0;
class _MyAcademicState extends State<MyAcademic> {
  ...
  Widget build(BuildContext context) {
    ...
    DatabaseReference reference = FirebaseDatabase.instance.ref("MyPost/$minemail/Academic");
    ...
    reference.orderByChild("ID").onChildAdded.listen((DatabaseEvent value) {
      ...
      Details de = Details(
          ...
          sb: false);
      setState(() {
        if (listam.length != myja) {
          if (idsam.contains(map["ID"])) {
          } else {
            idsam.insert(0, map["ID"]);
            listam.insert(0, de);
          }
        } else {
          if (update) {
            itemScrollController.jumpTo(index: mycindexa);
            mycindexa = 0;
            update = false;
          }
        }
    ...
    return countbool
      ...
      child: ScrollablePositionedList.builder(
          ...
          itemBuilder: (_, index) {
            String name = listam[index].Name;
            String photoUrl = listam[index].PhotoUrl;
            String emailId = listam[index].Email_ID;
            String hall = listam[index].Hall;
            String year = listam[index].Year;
            String department = listam[index].Department;
            String post = listam[index].Post;
            String iD = listam[index].ID;
            int comments = listam[index].Comment;
            String fileUrl = listam[index].FileUrl;
            bool showi = listam[index].showi;
            bool showd = listam[index].showd;
            ...
            geta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listam[index].lb = true;
              }else{
                listam[index].lb = false;
              }
            });
            ...
            FirebaseDatabase.instance.ref("Academic/$Departmentp/$iD");
            gh.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                Map<dynamic, dynamic> map = snap.value as Map;
                setState(() {
                  listam[index].Comment = map["Comment"];
                });
              }
            });
            ...
            beta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listam[index].sp = true;
              } else {
                listam[index].sp = false;
              }
            });
            ...
            bad.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listam[index].sb = true;
              } else {
                listam[index].sb = false;
              }
            });
            return Card(
              child: Column(
                ...
                children: [
                          ...
                          children: [
                            Padding(
                              ...
                              child: Row(
                                  ...
                                  ),listam[index].sb ? Image.asset('images/verr.png', width: 20, height: 20,) : SizedBox(width: 0,),
                                  ]
                              ),
                            ),
                            Text(
                            ...
                            onPressed: () async {
                              ...
                              Widget yes = TextButton(
                                ...
                                onPressed: () async {
                                  String? mymail = EmailIdp.replaceAll("@", '');
                                  String? minemail = mymail.replaceAll(".", "");
                                  DatabaseReference delref = FirebaseDatabase.instance.ref("MyPost/$minemail/Academic/$iD");
                                  await delref.remove();

                                  DatabaseReference delre = FirebaseDatabase.instance.ref("Academic/$department/$iD");
                                  await delre.remove();
                                  setState(() {
                                    listam.clear();
                                    idsam.clear();
                                    DatabaseReference reference =
                                    FirebaseDatabase.instance.ref("MyPost/$minemail/Academic");
                                    reference.once().then((value) {
                                      DataSnapshot snap = value.snapshot;
                                      Map<dynamic, dynamic> map = snap.value as Map;
                                      myja = map.length;
                                  ...
                                  setState(() {
                                    ...
                                    idsa.clear();
                                    DatabaseReference reference =
                                    FirebaseDatabase.instance.ref("Academic/$Departmentp");//academic path
                                    reference.once().then((value) {
                                      DataSnapshot snap = value.snapshot;
                                      Map<dynamic, dynamic> map = snap.value as Map;
                                      jor = map.length;
                  ...
                  Row(crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.start,children: [
                      ...
                      GestureDetector(
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(height: 5,),
                            Text(listam[index].like.toString(),
                      ...
                      listam[index].sp ? IconButton(onPressed: () {
                        setState(() {
                          DatabaseReference beta =
                          FirebaseDatabase.instance.ref("Saved/$minemail/$iD/");
                          beta.remove();
