...
class MyFunde extends StatefulWidget {
  const MyFunde({Key? key}) : super(key: key);

  @override
  State<MyFunde> createState() => _MyFundeState();
}
bool update = true;
int mycindexf = 0;
class _MyFundeState extends State<MyFunde> {
  ...
  Widget build(BuildContext context) {
    ...
    DatabaseReference reference = FirebaseDatabase.instance.ref("MyPost/$minemail/Funde");
    ...
    reference.orderByChild("ID").onChildAdded.listen((DatabaseEvent value) {
      ...
      Details de = Details(
          ...
          sb: false);
      setState(() {
        if (listfm.length != myj) {
          if (idsfm.contains(map["ID"])) {
          } else {
            idsfm.insert(0, map["ID"]);
            listfm.insert(0, de);
          }
        } else {
          if (update) {
            itemScrollController.jumpTo(index: mycindexf);
            mycindexf = 0;
            update = false;
          }
        }
    ...
    return countbool
      ...
      child: ScrollablePositionedList.builder(
          ...
          itemCount: listfm.length,
          itemBuilder: (_, index) {
            String name = listfm[index].Name;
            String photoUrl = listfm[index].PhotoUrl;
            String emailId = listfm[index].Email_ID;
            String hall = listfm[index].Hall;
            String year = listfm[index].Year;
            String department = listfm[index].Department;
            String post = listfm[index].Post;
            String iD = listfm[index].ID;
            int comments = listfm[index].Comment;
            String fileUrl = listfm[index].FileUrl;
            bool showi = listfm[index].showi;
            bool showd = listfm[index].showd;
            ...
            geta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listfm[index].lb = true;
              }else{
                listfm[index].lb = false;
              }
            });
            ...
            gh.once().then((value) async {
              ...
              if (snap.exists) {
                Map<dynamic, dynamic> map = snap.value as Map;
                setState(() {
                  listfm[index].Comment = map["Comment"];
                });
              }
            });
            ...
            beta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listfm[index].sp = true;
              } else {
                listfm[index].sp = false;
              }
            });
            ...
            bad.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listfm[index].sb = true;
              } else {
                listfm[index].sb = false;
              }
            });
            return Card(
              child: Column(
                ...
                children: [
                          ...
                          children: [
                            Padding(
                              ...
                              child: Row(
                                  ...
                                  ),listfm[index].sb ? Image.asset('images/verr.png', width: 20, height: 20,) : SizedBox(width: 0,),
                                  ]
                              ),
                            ),
                            Text(
                            ...
                            onPressed: () async {
                              ...
                              Widget yes = TextButton(
                                ...
                                onPressed: () async {
                                  String? mymail = EmailIdp.replaceAll("@", '');
                                  String? minemail = mymail.replaceAll(".", "");
                                  DatabaseReference delref = FirebaseDatabase.instance.ref("MyPost/$minemail/Funde/$iD");
                                  await delref.remove();
                                  ...
                                  setState(() {
                                    ...
                                    FirebaseDatabase.instance.ref("MyPost/$minemail/Funde");
                                    reference.once().then((value) {
                                      DataSnapshot snap = value.snapshot;
                                      Map<dynamic, dynamic> map = snap.value as Map;
                                      myj = map.length;
                  ...
                  Row(crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.start,children: [
                      ...
                      GestureDetector(
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(height: 5,),
                            Text(listfm[index].like.toString(),
                      ...
                      listfm[index].sp ? IconButton(onPressed: () {
                        setState(() {
                          DatabaseReference beta =
                          FirebaseDatabase.instance.ref("Saved/$minemail/$iD/");
                          beta.remove();
