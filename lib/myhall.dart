...
class MyHall extends StatefulWidget {
  const MyHall({Key? key}) : super(key: key);

  @override
  State<MyHall> createState() => _MyHallState();
}

class _MyHallState extends State<MyHall> {
  ...
  Widget build(BuildContext context) {
    ...
    DatabaseReference referenceh = FirebaseDatabase.instance.ref("MyPost/$minemail/Hall");
    ...
    referenceh.orderByChild("ID").onChildAdded.listen((DatabaseEvent value) {
      ...
      setState(() {
        ...
        if (listhm.length != myjo) {
          if(idshm.contains(map["ID"])){

          }else{
            idshm.insert(0, map["ID"]);
            listhm.insert(0, de);
          }
        }else{
          if (updateh) {
            itemScrollController.jumpTo(index: mycindexh);
            mycindexh = 0;
            updateh = false;
          }
        }
    ...
    return countbool
      ...
      child: ScrollablePositionedList.builder(
          ...
          itemCount: listhm.length,
          itemBuilder: (_, index) {
            String name = listhm[index].Name;
            String photoUrl = listhm[index].PhotoUrl;
            String emailId = listhm[index].Email_ID;
            String hall = listhm[index].Hall;
            String year = listhm[index].Year;
            String department = listhm[index].Department;
            String post = listhm[index].Post;
            String iD = listhm[index].ID;
            int comments = listhm[index].Comment;
            String fileUrl = listhm[index].FileUrl;
            bool showi = listhm[index].showi;
            bool showd = listhm[index].showd;
            ...
            geta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listhm[index].lb = true;
              }else{
                listhm[index].lb = false;
              }
            });
            ...
            gh.once().then((value) async {
              ...
              if (snap.exists) {
                Map<dynamic, dynamic> map = snap.value as Map;
                setState(() {
                  listhm[index].Comment = map["Comment"];
                });
              }
            });
            ...
            beta.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listhm[index].sp = true;
              } else {
                listhm[index].sp = false;
              }
            });
            ...
            bad.once().then((value) async {
              DataSnapshot snap = value.snapshot;
              if (snap.exists) {
                listhm[index].sb = true;
              } else {
                listhm[index].sb = false;
              }
            });
            return Card(
              child: Column(
                ...
                children: [
                          ...
                          children: [
                            Padding(
                              ...
                              child: Row(
                                  ...
                                  ),listhm[index].sb ? Image.asset('images/verr.png', width: 20, height: 20,) : SizedBox(width: 0,),
                                  ]
                              ),
                            ),
                            Text(
                              ...
                              Widget yes = TextButton(
                                ...
                                onPressed: () async {
                                  ...
                                  setState(() {
                                    ...
                                    FirebaseDatabase.instance.ref("MyPost/$minemail/Hall");
                                    referenceh.once().then((value) {
                                      DataSnapshot snap = value.snapshot;
                                      Map<dynamic, dynamic> map = snap.value as Map;
                                      myjo = map.length;
                  ...
                  Row(crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.start,children: [
                      ...
                      GestureDetector(
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(height: 5,),
                            Text(listhm[index].like.toString(),
                      ...
                      listhm[index].sp ? IconButton(onPressed: () {
                        setState(() {
                          DatabaseReference beta =
                          FirebaseDatabase.instance.ref("Saved/$minemail/$iD/");
                          beta.remove();
